...
def shortest(n, edges):
    ...
    ad_list = dict()
    hd = heapdict()
    visited = defaultdict(lambda: [float('inf'),0])

    for v, u, c in edges:
        if v not in ad_list: ad_list[v] = [(u, c)]
        else: ad_list[v].append((u, c))
        if u not in ad_list: ad_list[u] = [(v, c)]
        else: ad_list[u].append((v, c))
    ...
    while hd:
        ...
        for u, c in ad_list[curr]:
            ...
            if u not in visited and u not in hd:
                hd[u] = [c + dist, path + [u]]
                if hd[u][0] > c + dist:
                    hd[u] = [c + dist, path + [u]]
